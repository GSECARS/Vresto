...
class DiamondImagesGroupController(QObject):
    ...
    _omega_position_changed: Signal = Signal(float)
    ...
    def _connect_diamond_image_widgets(self) -> None:
        ...
        self._widget.btn_diamond_go.clicked.connect(
            lambda: self._move_focus(self._widget.lne_diamond_table, 2)
        )
        self._widget.btn_virtual_go.clicked.connect(
            lambda: self._move_focus(self._widget.lne_virtual_position, 1)
        )
        self._widget.btn_real_go.clicked.connect(
            lambda: self._move_focus(self._widget.lne_real_position, 3)
        )
        ...
        self._omega_position_changed.connect(self._update_diamond_images)
    ...
    def update_diamond_image_widgets(self) -> None:
        if self._epics.connected:
            ...
            if self._omega_stage.moving:
                self._omega_position_changed.emit(self._omega_stage.readback)
