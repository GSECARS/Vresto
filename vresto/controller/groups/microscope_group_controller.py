...
class MicroscopeGroupController(QObject):
    ...
    _microscope_in: float = 0.01
    _microscope_out: float = 0.02
    _microscope_zoom_in: float = 0.01
    _microscope_zoom_out: float = 0.02

    _slider_max: int = 100
    _slider_value_multiplier: int = 20

    _omega_limit: float = 120.0
    ...
    def _btn_microscope_in_clicked(self) -> None:
        ...
        if self._sample_omega_stage.readback != self._omega_limit:
            MsgBox(
                msg=f"First, move the stage in the MICROSCOPE position ({self._omega_limit})."
            )
            return None
